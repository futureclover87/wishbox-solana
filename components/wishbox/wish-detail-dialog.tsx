...
import type { Wish, WishStatus } from "./wish-card";
import { getCountdown } from "./wish-card";
import { WISHBOX_TREASURY, CLAIM_STAKE_RATIO } from "@/lib/wishbox-constants";
// CLAIM_STAKE_RATIO (10%) used only for transaction logic, not displayed
...
export function WishDetailDialog({
  ...
  const isBuilder =
    !!wish.builder && !!publicKey && publicKey.toBase58() === wish.builder;

  const stakeRequired = +(wish.bounty * CLAIM_STAKE_RATIO).toFixed(4);
  ...
  const catchTxError = (err: unknown) => {
  ...
  const handleContribute = async () => {
    ...
    setIsContributing(true); setTxError(null);
    try {
      const sig = await buildAndSendTx(Math.round(amount * LAMPORTS_PER_SOL));
    ...
    } catch (err) { catchTxError(err); }
    finally { setIsContributing(false); }
  };

  const handleClaim = async () => {
    if (!publicKey) return;
    setIsClaiming(true); setTxError(null);
    try {
      const sig = await buildAndSendTx(Math.round(stakeRequired * LAMPORTS_PER_SOL));
      console.log("[Wishbox] Claim stake tx:", sig);
      onClaim(wish.id);
      setClaimSuccess(true);
      setTimeout(() => { setShowClaimConfirm(false); setClaimSuccess(false); }, 3000);
    } catch (err) { catchTxError(err); }
  ...
  // Countdown for Open tasks
  const cd = wish.status === "Open" ? getCountdown(wish.deadline) : null;
  const cdColorMap = {
    normal: "text-foreground", warning: "text-amber-400",
    critical: "text-red-400", expired: "text-muted-foreground/50",
  } as const;
  const cdBorderMap = {
    normal: "border-glass-border", warning: "border-amber-500/30",
    critical: "border-red-500/40", expired: "border-glass-border",
  } as const;
      ...
      <DialogContent className="border-glass-border bg-background/95 backdrop-blur-xl sm:max-w-xl">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-lg border border-glass-border bg-secondary/30 p-4">
            ...
            <p className="font-mono text-xl font-bold text-primary">{wish.bounty} SOL</p>
          </div>
          <div className="rounded-lg border border-glass-border bg-secondary/30 p-4">
            <div className="mb-1 flex items-center gap-2 text-muted-foreground">
          ...
          {cd ? (
            <div className={`rounded-lg border bg-secondary/30 p-4 ${cdBorderMap[cd.level]}`}>
              <div className="mb-1 flex items-center gap-2 text-muted-foreground">
                <Timer className={`size-4 ${cdColorMap[cd.level]}`} />
                <span className="text-xs">Expires</span>
              </div>
              <p className={`text-xl font-bold ${cdColorMap[cd.level]}`}>{cd.label}</p>
        ...
        <div className="flex items-center justify-between rounded-lg border border-glass-border bg-secondary/20 p-3">
          <div className="flex items-center gap-2">
            {wish.isAnonymous ? (
              <><EyeOff className="size-4 text-accent" /><span className="text-sm text-accent">Anonymous</span></>
            ) : (
              <><Wallet className="size-4 text-primary" /><span className="text-sm text-muted-foreground">Verified Requester</span></>
            )}
          </div>
        ...
        <div className="mt-2 space-y-4">
          ...
          {isWalletConnected && wish.status === "Open" && (() => {
              ...
              <div key="contribute">
                {showContributeInput ? (
                  <div className="rounded-xl border border-primary/30 bg-primary/5 p-4">
                    <div className="mb-4 flex items-center justify-between">
                      <h3 className="flex items-center gap-2 text-lg font-semibold text-foreground">
                        <Plus className="size-5 text-primary" /> Contribute to Task
                      </h3>
                    ...
                    {contributeSuccess ? (
                      <div className="flex flex-col items-center py-4">
                        ...
                        <p className="text-sm text-muted-foreground">You contributed {contributeAmount} SOL</p>
                      </div>
                    ) : (
                      <>
                        <p className="mb-4 text-sm text-muted-foreground">
                          Add more SOL to increase the bounty pool and attract skilled contributors.
                        </p>
                        <div className="flex gap-2">
                          ...
                          <Button onClick={handleContribute}
                            ...
                            {isContributing ? <Loader2 className="size-5 animate-spin" /> : <><span>Confirm</span><ArrowRight className="ml-2 size-4" /></>}
                          </Button>
                        </div>
                        <div className="mt-3 flex gap-2">
                          {[0.1, 0.5, 1, 2].map((a) => (
                            <button key={a} onClick={() => setContributeAmount(a.toString())}
                  ...
                  <Button onClick={() => setShowContributeInput(true)}
                    ...
                    <Plus className="mr-2 size-6" /> Contribute to This Task
                  </Button>
                ) : (
                  <Button variant="outline" onClick={() => setShowContributeInput(true)}
                    ...
                    <Plus className="mr-2 size-4" /> Contribute to This Task
                  </Button>
                )}
              </div>
            );
              ...
              <div key="claim">
                {showClaimConfirm ? (
                  <div className="rounded-xl border-2 border-accent/50 bg-accent/5 p-6">
                    <div className="mb-4 flex items-center justify-between">
                      <h3 className="flex items-center gap-2 text-xl font-bold text-accent">
                        <HandHeart className="size-6" /> Claim This Task
                      </h3>
                      <Button variant="ghost" size="icon"
                    ...
                    {claimSuccess ? (
                      <div className="flex flex-col items-center py-6">
                        ...
                        <p className="mt-2 text-center text-muted-foreground">
                          You are now responsible for completing this task.
                        </p>
                      </div>
                    ) : (
                      <>
                        <div className="mb-4 rounded-lg border border-glass-border bg-secondary/20 p-4">
                          ...
                          <ul className="space-y-2 text-sm text-muted-foreground">
                            ...
                            <li className="flex items-start gap-2"><CheckCircle2 className="mt-0.5 size-4 shrink-0 text-accent" />Receive the bounty ({wish.bounty} SOL) upon approval</li>
                          </ul>
                        </div>
                        <Button onClick={handleClaim} disabled={isClaiming}
                          ...
                          {isClaiming
                            ? <><Loader2 className="mr-2 size-6 animate-spin" />Waiting for signature…</>
                            : <><HandHeart className="mr-2 size-6" />Claim This Task</>}
                        </Button>
                  ...
                  <button onClick={() => setShowClaimConfirm(true)}
                    ...
                    <div className="relative flex items-center gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="mb-3 text-muted-foreground">Take responsibility and earn the bounty</p>
                        <div className="flex flex-wrap items-center gap-3">
                          <div className="flex items-center gap-2 rounded-lg bg-accent/20 px-4 py-2">
                            <Coins className="size-5 text-accent" />
                            <span className="font-mono text-xl font-bold text-accent">{wish.bounty} SOL</span>
                          </div>
                  ...
                  <Button variant="outline" onClick={() => setShowClaimConfirm(true)}
                    ...
                    <HandHeart className="mr-2 size-4" /> Claim This Task
                    <span className="ml-auto text-xs opacity-60">{wish.bounty} SOL bounty</span>
                  </Button>
                )}
              </div>
            );
            ...
            return (
              <div className="flex flex-col gap-4">
                {userRole === "requester"
                  ? <>{contributeBlock}{claimBlock}</>
                  : <>{claimBlock}{contributeBlock}</>
                }
              </div>
            );
          })()}

          {/* ── Accepted: claimer vs others ── */}
          {isWalletConnected && wish.status === "Accepted" && (
            isBuilder ? (
                ...
                <div className="mb-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      You&apos;re working on it — open the task dialog to submit your delivery.
                    </p>
                  </div>
                </div>
                <div className="rounded-lg border border-accent/20 bg-accent/5 px-4 py-3 text-sm text-accent/80 flex items-center gap-2">
                  <CheckCircle2 className="size-4 shrink-0" />
                  You are committed to completing this task. Submit your delivery when ready.
                </div>
              </div>
            ) : (
          ...
          {isWalletConnected && wish.status === "Settled" && (
            <div className="rounded-xl border border-primary/30 bg-primary/10 p-6 text-center">
              ...
              <h3 className="mb-2 text-xl font-bold text-primary">Bounty Settled</h3>
              <p className="text-muted-foreground">
                This task has been completed and the bounty has been distributed.
              </p>
              <div className="mt-4 flex items-center justify-center gap-1.5 text-sm text-muted-foreground">
